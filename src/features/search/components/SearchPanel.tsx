...
function SearchTabsHeader({ totalIndexedPapers }: { totalIndexedPapers: number }) {
  // Format display text before JSX so the markup stays simple.
  const indexedPaperLabel =
    totalIndexedPapers > 0
      ? `${formatCompactNumber(totalIndexedPapers)} papers indexed`
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl bg-white p-3 shadow-sm ring-1 ring-slate-200 focus-within:ring-emerald-700 md:flex-row md:items-center">
      ...
      <button
        ...
        className="inline-flex items-center justify-center gap-2 rounded-lg bg-emerald-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-emerald-800 disabled:cursor-not-allowed disabled:bg-slate-400"
      >
...
function SavedSearchButton({
  ...
  return (
    <div className="flex items-center rounded-xl transition hover:bg-emerald-50">
      <button
        type="button"
        onMouseDown={keepSearchInputFocused}
        onClick={handleSavedSearchClick}
        className="flex min-w-0 flex-1 items-center gap-3 px-3 py-2.5 text-left text-sm font-semibold text-slate-800 transition hover:text-emerald-950"
      >
      ...
      <button
        ...
        className="mr-2 flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-slate-400 transition hover:bg-red-50 hover:text-red-600"
      >
        <X className="h-4 w-4" />
      </button>
    </div>
